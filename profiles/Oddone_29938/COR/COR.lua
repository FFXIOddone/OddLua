...
mechanicsSwapPlanner.transitions['Roll'] = {
    ['sourceSet'] = 'Aftercast',
    ['targetSet'] = 'Roll',
    ['actions'] = {
        {
...
local function mainJobHasNativeDualWield()
    return nativeDualWieldMainJobs['COR'] == true;
end
...
local function equipCombatStyle(force)
    if state.Playstyle == 'Craft' and isEngaged(getPlayer()) then
        ...
        if equipNamedSet(setNameFor('Roll'), force) then
            return true;
        end
    ...
    return equipNamedSet('Playstyle_Roll', force);
end

local function lockstyleCombatSet()
    if not equipNamedSet('TP', true) then
        if not equipNamedSet(setNameFor('Roll'), true) then
...
local oddLuaNumberRow = {
    renderEvent = 'oddlua_number_row_Oddone_29938_COR',
    imgui = nil,
...
function oddLuaNumberRow.renderOverlay()
    ...
    local title = 'OddLua Oddone_29938 COR';
