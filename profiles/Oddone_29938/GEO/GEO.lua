...
local function equipCurrent(force)
    if state.Playstyle == 'Craft' and isEngaged(getPlayer()) then
        ...
        equipNamedSet(setNameFor('GeoMagic'), force);
        return;
    end
...
profile.OnLoad = function()
    ...
    message('OddLua playstyle swapper loaded for Oddone_29938. Default style: ' .. state.Playstyle .. '. Use /lac fwd style geomagic|nuke|fastcast|idlerefresh.');
end
...
profile.HandleCommand = function(args)
    ...
    if command == 'style' or command == 'playstyle' then
        if value == '' or value == 'status' then
            message('Current style: ' .. state.Playstyle .. '. Use style geomagic|nuke|fastcast|idlerefresh.');
            return;
        end

        local selected = styleAliases[value];
        if not selected then
            message('Unknown style: ' .. tostring(args[2]) .. '. Use style geomagic|nuke|fastcast|idlerefresh.');
            return;
        end
...
profile.HandleWeaponskill = function()
    ...
    elseif sets.Playstyle_GeoMagic then
        equipNamedSet('Playstyle_GeoMagic', false);
    end
end
